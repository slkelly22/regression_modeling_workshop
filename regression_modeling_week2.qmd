...
You will have access to the `gss_cat` data that lives inside the `forcats` package (part of tidyverse).
...
I want you to create a linear model predicting `tvhours`. Use `age` and an additional factor (your choice) as predictors. Be sure to check the levels of your factor so you can interpret the coefficients.
...
> We're going to work through models with binary outcomes -- logistic regression!
